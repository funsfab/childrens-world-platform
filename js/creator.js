...
const ROOM_REQUIREMENTS={
 ...
 lounge:{sofa:1,'coffee-table':1,television:1,'tv-stand':1},
 dining:{'dining-table':1,'dining-chair':2,pendant:1},
 kitchen:{'kitchen-counter':1,'kitchen-sink':1,hob:1,fridge:1},
 style:{'floor-lamp':1,'wall-art':1,curtains:1},
 smart:{'smart-speaker':1,'robot-vacuum':1,'smart-light':1}
};
...
function refreshRoomComponentButtons(){
 ...
 document.querySelectorAll('#creatorComponents .creator-component[data-part-id]').forEach(b=>{
   const id=b.dataset.partId,req=roomModeRequirements(activeMode),need=req[id]||0,done=roomPartCount(id),complete=need>0&&done>=need;
   b.classList.toggle('room-complete',complete);
   if(need){b.disabled=complete;b.setAttribute('aria-disabled',complete?'true':'false');const c=b.querySelector('[data-room-counter]');if(c)c.textContent=complete?`✓ ${done}/${need}`:`${done}/${need}`;}
 });
}
function updateRoomProgressUI({autoAdvance=false}={}){
 if(active!=='room')return;
 const ready=roomReadyForTest(),test=$('testCreation'),status=$('roomBuildStatus');
 if(test){test.disabled=!ready;test.setAttribute('aria-disabled',ready?'false':'true');test.title=ready?'':t('Complete every Dream Room build area first.','Termine d’abord toutes les zones de la Chambre de rêve.');}
 if(status)status.textContent=ready?t('✓ Room build complete — ready for live-use test','✓ Pièce terminée — prête pour le test en situation'):t('Complete each room zone to unlock Test My Creation.','Termine chaque zone pour débloquer Tester ma création.');
 document.querySelectorAll('#creatorModes .creator-mode[data-mode-id]').forEach(b=>{const done=roomModeComplete(b.dataset.modeId);b.classList.toggle('room-complete',done);let st=b.querySelector('.creator-mode-status');if(done&&!st){st=document.createElement('small');st.className='creator-mode-status';st.textContent='✓ '+t('Complete','Terminé');b.appendChild(st)}if(!done&&st)st.remove();});
 refreshRoomComponentButtons();
 if(autoAdvance&&roomModeComplete(activeMode)){
   const next=roomNextUnfinishedMode();
   if(next&&next!==activeMode){activeMode=next;renderModes(missions.room,next);renderTemplate('room');$('creatorCoach').textContent=t('✓ Section complete. Moving to the next unfinished room zone.','✓ Section terminée. Passage à la prochaine zone inachevée.');}
   else if(!next){$('creatorCoach').textContent=t('Dream Room build complete. Test My Creation is ready.','Construction de la Chambre de rêve terminée. Tester ma création est prêt.');}
 }
}
...
function startRoomLiveTest(){
  if(!roomReadyForTest()){
    const next=roomNextUnfinishedMode(),label=roomModes.find(x=>x.id===next)?.label||[next,next];
    $('creatorTestResult').innerHTML=`<div class="creator-report warning"><b>${t('Finish the Dream Room build first','Termine d’abord la Chambre de rêve')}</b><p>${t('Next unfinished zone','Prochaine zone inachevée')}: ${L(label)}. ${roomRequirementText(next)}</p></div>`;
    $('creatorCoach').textContent=t('Test My Creation unlocks only when every required room zone is complete.','Tester ma création se débloque seulement lorsque chaque zone requise est terminée.');
    updateRoomProgressUI();return;
  }
...
function updateBlueprintVisibility(){
  ...
  const generic=$('toggleTemplate'),vehicleToggle=$('vehicleToggleTemplate'),roomToggle=$('roomToggleTemplate'),roomControls=$('roomViewControls'),drawingToolbar=$('creatorDrawingToolbar');
  const label=templateOn?t('👻 Blueprint on','👻 Plan activé'):t('👻 Blueprint off','👻 Plan désactivé');
  if(generic){generic.textContent=label;generic.hidden=active==='vehicle'||active==='room';}
  if(vehicleToggle){vehicleToggle.textContent=label;vehicleToggle.classList.toggle('active',templateOn);}
  if(roomToggle){roomToggle.textContent=label;roomToggle.classList.toggle('active',templateOn);}
  if(roomControls)roomControls.hidden=active!=='room';
  if(drawingToolbar)drawingToolbar.hidden=(active==='vehicle'||active==='room')&&templateOn;
...
function renderLibrary(m){
  ...
  if(active==='room'){
    $('creatorLibraryTitle').textContent=t('Choose your Dream Room style.','Choisis le style de ta Chambre de rêve.');
    $('creatorLibraryHelp').textContent=t('One room preview is shown at a time. Use the compact list to switch styles without making the page longer.','Un seul aperçu est affiché à la fois. Utilise la liste compacte pour changer de style sans rallonger la page.');
    const current=roomLibrary.find(v=>v.id===currentLibraryId)||roomLibrary[0];currentLibraryId=current.id;
    const opts=roomLibrary.map(v=>`<option value="${v.id}" ${v.id===currentLibraryId?'selected':''}>${v.name}</option>`).join('');
    host.className='creator-library-grid creator-room-picker';
    host.innerHTML=`<div class="creator-room-preview"><img src="${current.img}" alt="${current.name}" width="960" height="640" loading="eager" decoding="async" fetchpriority="high"><div class="creator-room-preview-copy"><span><b>${current.name}</b><small>${current.subtitle}</small></span><strong>${t('Selected','Sélectionné')} ✓</strong></div></div><div class="creator-room-picker-control"><label for="roomPickerSelect">${t('Room style list','Liste des styles')}</label><select id="roomPickerSelect">${opts}</select><small>${t('The style changes the visual direction. Your placed furniture and saved room progress stay in place.','Le style change la direction visuelle. Les meubles placés et la progression enregistrée restent en place.')}</small></div>`;
    const select=$('roomPickerSelect');if(select)select.onchange=()=>{const item=roomLibrary.find(v=>v.id===select.value);if(item)chooseLibrary(item)};
    return;
  }
...
if(active==='room'){
  ...
  $('creatorCoach').textContent=t('Style selected. Your room build stays intact. Continue with the current unfinished zone.','Style sélectionné. Ta construction reste intacte. Continue avec la zone inachevée actuelle.');
  commitHistory();return;
}
...
function renderModes(m,preferred){
  ...
  modes.forEach(md=>{
    const complete=active==='vehicle'?vehicleModeComplete(md.id):(active==='room'?roomModeComplete(md.id):false);
  ...
  if(active==='room'){renderTemplate('room');updateRoomProgressUI();}
}
...
function addPalettePart(p){
  ...
  if(active==='room'){const need=roomModeRequirements(activeMode)[p.id]||0;if(need&&roomPartCount(p.id)>=need)return;}
...
function renderComponents(md){
  ...
  if(active==='room'){
    const req=roomModeRequirements(md.id);
    host.innerHTML=`<div class="creator-palette-title"><b>${t('Build','Construis')} ${L(md.label)}</b><small>${t('Required for this zone','Requis pour cette zone')}: ${roomRequirementText(md.id)}</small></div>`;
    (md.parts||[]).forEach(p=>{const b=document.createElement('button');b.type='button';b.className='creator-component';b.dataset.partId=p.id;const need=req[p.id]||0,done=roomPartCount(p.id);b.innerHTML=`<span>${paletteVisual(p)}</span><small>${L(p.label)}${need?` <em data-room-counter>${done}/${need}</em>`:''}</small>`;if(need&&done>=need){b.disabled=true;b.classList.add('room-complete')}b.onclick=()=>addPalettePart(p);host.appendChild(b)});
    refreshRoomComponentButtons();return;
  }
...
const toggleBlueprint=()=>{
  templateOn=!templateOn;if((active==='vehicle'||active==='room')&&templateOn)setTool('select');renderTemplate(active);updateBlueprintVisibility();commitHistory();
  if(active==='vehicle')showVehicleStageNotice(templateOn?t('Build mode restored','Mode construction restauré'):t('Creative drawing tools available','Outils de dessin disponibles'),templateOn?t('Fit the matching vehicle pieces to the blueprint.','Place les pièces correspondantes sur le plan.'):t('The blueprint is hidden. Draw or decorate freely, then turn it back on to continue building.','Le plan est masqué. Dessine ou décore librement, puis réactive-le pour continuer la construction.'),{duration:3000});
  if(active==='room'){$('creatorCoach').textContent=templateOn?t('Blueprint ON: arrange real room objects and complete each guided zone. Drawing and colour tools are hidden.','Plan activé : organise les vrais objets et termine chaque zone guidée. Les outils de dessin et de couleur sont masqués.'):t('Blueprint OFF: free drawing and decoration tools are available. Your placed room objects stay saved.','Plan désactivé : les outils de dessin et décoration sont disponibles. Les objets placés restent enregistrés.');}
};$('toggleTemplate').onclick=toggleBlueprint;if($('vehicleToggleTemplate'))$('vehicleToggleTemplate').onclick=toggleBlueprint;if($('roomToggleTemplate'))$('roomToggleTemplate').onclick=toggleBlueprint;
